...
- **Headless mirror runner** — for large one-time backfills, prefer `pnpm backfill:headless -- --samples-dir /Volumes/disco/splicerr --batch-size 1000 --concurrency 16`. It uses Playwright only for Splice GraphQL (browser-origin fetch), then downloads, descrambles, writes MP3s, and upserts SQLite directly from Node. It reuses `mirror_pack_queue`, resets interrupted `listing`/`downloading` packs to `queued` on startup, and avoids Svelte/UI reactive state.
