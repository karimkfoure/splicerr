...
- Library browsing: My library has no page number or SQL `OFFSET`. It uses opaque keyset cursors, silently prefetches bounded SQLite windows, and virtualizes rows so only the visible range is mounted. Splice keeps its separate `PER_PAGE` pagination. Filtered local windows use `LIMIT + 1` instead of blocking on exact `COUNT(*)`; unfiltered totals remain exact through `library_stats`.
