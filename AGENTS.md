...
- **MP3 padding and permanent WAV export** — Splice preview MP3s sampled from the mirror are raw CBR streams without Xing/LAME gapless metadata. Deterministic analysis of 10k loops and 10k one-shots found the dominant decoded-content transition at 1105 samples regardless of 44.1/48k sample rate; correlation against 499 cached Splice waveform sidecars favored 1105 over 0/529/576/1152 in 389 cases and raised median correlation from 0.618 (no trim) to 0.916. Use `pnpm analyze:mp3-padding -- --category=loop --limit=10000`, `--category=oneshot`, and `pnpm analyze:waveform-alignment -- --fine=true` to reproduce. [`audio_export.rs`](src-tauri/src/audio_export.rs) is the versioned source of truth: drag, Export WAV, and favoriting atomically overwrite a 24-bit WAV at `samples_dir/exported/{original relative tree}.wav`; existing favorites are reconciled on library open. The default start trim is 1105 decoded samples. Loops within 0.05 beat of an integer export at the exact BPM-derived frame length; one-shots and ambiguous loops preserve the remaining tail. The always-visible `WAV correction` switch is diagnostic only: off means no trim and no BPM-length correction, never the removed upstream 12ms rule; drag logs the complete export decision. Preserve original MP3s and do not persist per-sample telemetry by default; policy inputs derive from existing duration/BPM and MP3 headers.
