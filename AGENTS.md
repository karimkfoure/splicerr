...
- **Local spectral waveforms** — library rows render 160 frequency-colored bins computed from the cached MP3 by [`audio_waveform.rs`](src-tauri/src/audio_waveform.rs); low/mid/high energy maps to warm/green/blue. The UI does not consume Splice waveform sidecars, but existing sidecars remain untouched. Analysis runs on blocking workers with frontend concurrency 2, an 80ms anti-scroll debounce, a 600-entry in-memory LRU, and one canvas per visible row; do not add per-sample waveform blobs to SQLite or millions of cache files without fresh measurements. The deterministic 40-file release benchmark measured decode p50/p95 11.77/35.24ms and analysis p50/p95 0.69/5.73ms.
