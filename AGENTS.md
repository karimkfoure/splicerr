...
- **Headless/app schema overlap** — maintenance scripts may create a column or index before the Tauri migration marker is written. Column-adding migrations must therefore check `pragma_table_info` and create remaining indexes idempotently before recording their version; never treat an SQLite-open/migration failure as an invalid Samples Directory.
- **Offline popularity enrichment** — `pnpm backfill:popularity -- --samples-dir /Volumes/disco/splicerr` traverses remote samples by popularity without downloading audio. A pack's offline proxy is the reciprocal rank of its first observed sample. The runner only records packs present in `library_pack_counts`, checkpoints every 25 GraphQL pages, resumes after interruption, and stops once all local packs have scores or the remote listing ends. `--restart` deliberately starts a fresh snapshot; `--dry-run --max-batches 2` probes GraphQL without changing scores.
- **Pack popularity catalog probe** — prefer `pnpm backfill:pack-popularity -- --samples-dir /Volumes/disco/splicerr` before using the sample-derived proxy. It uses the same popularity-sorted pack query as the app, stores `popularity_rank` once in `packs`, checkpoints each page, and probes beyond reported `totalPages` until the server returns an empty, repeated, or clamped page. It never writes sample rows or downloads audio; its completion log is the evidence for whether the pack endpoint can cover the full local catalog.
- **In-app popularity super sync** — Settings exposes the same official pack-ranking traversal through the authenticated hidden GraphQL webview. It starts a fresh snapshot, checkpoints each 100-pack page in `pack_popularity_backfill_checkpoint`, shows local coverage and the last completed sync, and deliberately probes beyond reported pages until the endpoint terminates. Keep the in-app and `backfill:pack-popularity` completion timestamp semantics aligned.
- **MP3 padding and permanent WAV export** — Deterministic analysis of 10k loops and 10k one-shots found the dominant decoded-content transition at 1105 samples regardless of 44.1/48k sample rate; correlation against 499 cached Splice waveform sidecars raised median alignment correlation from 0.618 (no trim) to 0.916. A separate 10k-header audit found 9583 MP3s without declared delay and 416 declaring exactly 1105 (one unreadable cached row); a 40-file FFmpeg/Symphonia comparison proved Symphonia preserves the leading skip in both cohorts. Use `pnpm analyze:mp3-padding -- --category=loop --limit=10000`, `--category=oneshot`, `pnpm analyze:waveform-alignment -- --fine=true`, and `pnpm audit:exports` to reproduce the relevant checks. [`audio_export.rs`](src-tauri/src/audio_export.rs) policy v2 is the versioned source of truth: use a sane declared MP3 delay when present, otherwise the audited 1105 fallback. Drag, Export WAV, and favoriting atomically overwrite a 24-bit WAV at `samples_dir/exported/{original relative tree}.wav`; all favorite WAVs regenerate once per app launch so policy changes replace stale exports. Loops within 0.05 beat of an integer export at the exact BPM-derived frame length; after full head trim, a rare shortfall of at most one MP3 frame is zero-padded at the end rather than retaining decoder padding at the start. Larger contradictions are not duration-forced. One-shots and ambiguous loops preserve their remaining tail. Ableton ignores ACID WAV tempo tags, so exact frame length is the BPM contract. The always-visible `WAV correction` switch is diagnostic only: off means no trim and no BPM-length correction, never the removed upstream fixed-millisecond rule; drag logs the complete export decision. Preserve original MP3s and do not persist per-sample telemetry by default.
- **Local spectral waveforms** — library rows render 320 frequency-colored bins computed from the cached MP3 by [`audio_waveform.rs`](src-tauri/src/audio_waveform.rs); low/mid/high energy maps to warm/green/blue, and the canvas follows its actual device-pixel dimensions. The UI does not consume Splice waveform sidecars, but existing sidecars remain untouched. Analysis runs on blocking workers with frontend concurrency 2, an 80ms anti-scroll debounce, cancellation for queued rows that leave the virtual viewport, and a 600-entry in-memory LRU. Results persist lazily in the separate `.splicerr/waveforms.db` as packed RGB (960 payload bytes/sample), keyed by a stable 64-bit path hash and validated against algorithm version plus source size/mtime; this keeps `library.db` and browse queries untouched, creates no per-sample files, and makes the cache disposable. `pnpm backfill:waveforms -- --samples-dir /Volumes/disco/splicerr` uses that same Rust analyzer for a bounded, resumable eager fill; add `--rebuild` to clear derived rows first after an algorithm change, and use `--limit` for probes. A deterministic 100k-row SQLite probe used 102.7MB total, projecting about 3.08GB only if all 3M samples are eventually viewed; do not eagerly backfill the full mirror without fresh measurements. The deterministic 40-file release benchmark measured an 815.4ms cold pass versus 78.5ms hot (1.963ms/sample); cold decode p50/p95 was 9.47/38.06ms and analysis p50/p95 0.78/8.87ms. Use `pnpm test:waveform-scheduler` to reproduce the fast-scroll cancellation invariant.
...
- **Initial browse ownership** — `+page.svelte` owns the one initial fetch and waits for `configLoadState.loaded`, which is set only after `loadConfig` has validated the directory and opened SQLite. Keep the page rendered while this happens; `+layout.svelte` must not issue a competing compensatory fetch.
...
- Filters (text, category, pack, key, BPM, tags): every change goes through `reloadFilteredAssets()` (`resetAssetList()` + `fetchAssets()`) so local cursors and in-flight pages cannot leak across identities. Tag-only filtering preserves the indexed official `pack_popularity` order. Text search remains FTS-driven even when that sort is selected: materializing and re-sorting broad FTS matches by pack rank measured tens of seconds on the 3.3M cache, while FTS order is interactive.
