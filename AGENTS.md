...
- **Derived local metadata** — `backfill-library-covers.mjs` downloads missing `cover_source_url` files atomically and marks `packs.cover_cached_at`; `backfill-library-bitrates.mjs` derives average kbps from MP3 byte size and stored duration into `samples.bitrate_kbps`. Both walk indexed pending rows in UUID order, commit by batch, and can be restarted. My library must consume these local values only; do not fetch covers or stat audio during browsing.
...
- Library pagination: `LIBRARY_PER_PAGE` is an internal SQLite window, not a visible interaction; prefetch local windows several viewports before the end. Splice stays `PER_PAGE` + infinite scroll. Filtered library pages use `LIMIT + 1` and progressive `N+` totals instead of blocking on exact `COUNT(*)`; unfiltered totals remain exact through `library_stats`.
